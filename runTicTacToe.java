...
public class runTicTacToe {
	...
	public void run()
	{
		...
		printBoardTicTacToe(board);
		 //1 = player1's turn, 2 = player2's turn, who go first is randomized 
		int counter=0, win1=0, win2=0,draw=0, totaltime=0;
		
		
		while(counter<1) {
		  Random rand = new Random();
		  int turn = rand.nextInt(2)+1;		  
		  int nummoves=0,time=0;
		...
		while((result = isEnded())==0) //game loop
		{
		   System.out.println("Player "+turn+" :");
			if(turn==1)
			{
				...
				System.out.println("move: ("+player1NextMove.x+","+player1NextMove.y+","+player1NextMove.z+")");
			...
			else if(turn==2)
			{
			  ...
			  System.out.println("Enter X,Y,Z coordinates with zero indexing (X is the row, Y is the column, Z is the level): ");
			  ...
			  while(true){
				  ...
				  if(x>=0 && x<4 && y>=0 && y<4 && z>=0 && z<4)break;
				  
			  }


			  positionTicTacToe player2NextMove = new positionTicTacToe(x,y,z);
		...
		totaltime+=(time/nummoves);
		board=createTicTacToeBoard();
		counter++;
		}
		
		System.out.println("efficiency: "+(totaltime/100)+"\n");
		System.out.println("win1: "+win1+" win2: "+win2+" draw: "+draw+" win rate: "+(win1/100));
